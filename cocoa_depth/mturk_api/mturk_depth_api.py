...
#HOST_DOMAIN = 'https://54.201.101.50'
...
def getHITType():
    ...
    _keywords = "person, people, image, images, object, objects, depth, comparisons, coco"
    
    _reward = _mtc.get_price_as_price(0.05)
    _duration = 60 * 15
...
def createHITs( savePath = '/home/ubuntu/amt_guis/cocoa_depth/hits/coco/', hit_name = '' ):
    ...
    for setId in setIds:
        ...
        external_url = HOST_DOMAIN + '/cocoa_depth/coco/' + str( setId )
        print external_url
...
def processAssignments( save = True, savePath = '/home/ubuntu/amt_guis/cocoa_depth/hits/coco/', hit_name = '', verbose = False ):
    ...
    for ass_data in assignments_data:
 	 ...
 	 #print _trials_results_dict[_trials_results_dict.keys()[0]].keys()
         if "_coco_subj_id" not in _trials_results_dict[_trials_results_dict.keys()[0]].keys():
             continue
...
def cleanAssignmentData( ass_data ):
    ...
    _ass_coco_subj_ids = _mongo_coll_3.find_one({'_amt_hit_id':_polished_data['hit_id']})['_coco_subjs_ids']
    _polished_data['coco_subj_ids'] = _ass_coco_subj_ids

    #_ass_coco_img_ids = [_mongo_coll_2.find_one({'_coco_subj_id':x})['_coco_img_id'] for x in _ass_coco_subj_ids]
    #_polished_data['coco_img_ids'] = _ass_coco_img_ids
	...
	res_coll_1 = _mongo_coll_1.find_one({ '_coco_subj_id':_trial['_coco_subj_id'] })
	_trial_info['img_id'] = res_coll_1['_coco_img_id']
	#_ass_trials[_trial['_coco_subj_id']] = \
        #{'coco_subj_id': _trial['_coco_subj_id'],
...
def extractAssignmentData( assignments, verbose = False ):
    ...
    # These are the contents of the assignments for the interaction HITs
    _worker_id = ''
    _worker_exp = 0
    _hit_id = 0
    _assignment_id = ''
